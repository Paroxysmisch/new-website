...
def render_inline_math(match: re.Match[str]) -> str:
    ...
    typ = f"${src}$"
    return (
        '<span class="typst-math transition-all border-2 border-transparent transition-colors duration-800 rounded-md hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50" data-typst="{html.escape(typ)}">'
...
def render_block_math(match: re.Match[str]) -> str:
    ...
    typ = f"$ {src} $"
    return (
        f'<div class="typst-math transition-all border-2 border-transparent transition-colors duration-800 rounded-md hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50" data-typst="{html.escape(typ)}">'
        + fix_svg(typst_compile(typ))
        + for_screen_reader(typ)
        + "</div>"
    )
